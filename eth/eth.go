...
type BlockByNumber struct {
	...
	adapter    eetypes.CosmosTxAdapter
}

func NewBlockByNumber(blockStore store.BlockStoreReader, adapter eetypes.CosmosTxAdapter) *BlockByNumber {
...
func NewBlockByHash(blockStore store.BlockStoreReader, adapter eetypes.CosmosTxAdapter) *BlockByHash {
	return &BlockByHash{
		blockStore: blockStore,
		adapter:    adapter,
	}
